...
from assets.robot_cfg import ROBOT_CONFIG
...
class NewRobotsSceneCfg(InteractiveSceneCfg):
    ...
    # robot — TODO: adjust prim_path and init_state to match your robot
    robot = ROBOT_CONFIG.replace(prim_path="{ENV_REGEX_NS}/Robot")
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    while simulation_app.is_running():
        ...
        if count % 500 == 0:
            # reset counters
            count = 0
            print("joint_names:", scene["robot"].data.joint_names)
            print("-------------------------------------------------------")
            print("body_names:", scene["robot"].data.body_names)
            print("-------------------------------------------------------")
        ...
        # scene["robot"].set_joint_velocity_target(action)
